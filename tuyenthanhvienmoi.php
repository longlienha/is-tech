<?php
    ...
    if(isset($_POST['submit'])){
        ...
        empty($_POST['major']) or empty($_POST['class']) or empty($_POST['phone'])){
            ...
            $unit = mysqli_real_escape_string($conn, $_POST['department']);
            
            if(strlen($phone) < 10 ){
                echo'<p style = "color:red">Số điện thoại không chính xác</p>';
            }else{
                    $sql = "SELECT * FROM `tuyenthanhvien` WHERE `email` like '$email' or `studentid` like '$student_id'";
                    $query = mysqli_query($conn,$sql);
                    $num = mysqli_num_rows($query);
                    if($num == 0){
                        $sql2= "INSERT INTO `tuyenthanhvien` (`st_name`, `sex`, `birthdate`, `city`, `fb`, `email`, `isvnu`, `studentid`, `major`, `class`, `phone`, `info`,`unit`) VALUES (N'$name','$sex','$birth_date',N'$city','$fb','$email','$isvnu','$student_id','$major','$class','$phone',N'$infor','$unit')";
